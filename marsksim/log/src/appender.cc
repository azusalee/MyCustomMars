...
#include "marsksim/log/appender.h"
#include <stdio.h>
...
#include "marsksim/comm/thread/condition.h"
#include "marsksim/comm/thread/thread.h"
#include "marsksim/comm/scope_recursion_limit.h"
#include "marsksim/comm/bootrun.h"
#include "marsksim/comm/tickcount.h"
#include "marsksim/comm/autobuffer.h"
#include "marsksim/comm/ptrbuffer.h"
#include "marsksim/comm/xlogger/xloggerbase.h"
#include "marsksim/comm/time_utils.h"
#include "marsksim/comm/strutil.h"
#include "marsksim/comm/mmap_util.h"
#include "marsksim/comm/tickcount.h"
#include "marsksim/comm/verinfo.h"
...
static boost_ksim::iostreams::mapped_file& sg_mmmap_file = *(new boost_ksim::iostreams::mapped_file);
...
static void __get_filenames_by_prefix(const std::string& _logdir, const std::string& _fileprefix, const std::string& _fileext, std::vector<std::string>& _filename_vec) {
    ...
    boost_ksim::filesystem::path path(_logdir);
    if (!boost_ksim::filesystem::is_directory(path)) {
        return;
    }
    
    boost_ksim::filesystem::directory_iterator end_iter;
    std::string filename;
    
    for (boost_ksim::filesystem::directory_iterator iter(path); iter != end_iter; ++iter) {
        if (boost_ksim::filesystem::is_regular_file(iter->status())) {
...
static long __get_next_fileindex(const std::string& _fileprefix, const std::string& _fileext) {
    ...
    if (boost_ksim::filesystem::exists(logfilepath)) {
        filesize += boost_ksim::filesystem::file_size(logfilepath);
    }
    if (!sg_cache_logdir.empty()) {
...
static void __del_timeout_file(const std::string& _log_path) {
    ...
    boost_ksim::filesystem::path path(_log_path);
    
    if (boost_ksim::filesystem::exists(path) && boost_ksim::filesystem::is_directory(path)){
        boost_ksim::filesystem::directory_iterator end_iter;
        for (boost_ksim::filesystem::directory_iterator iter(path); iter != end_iter; ++iter) {
            time_t file_modify_time = boost_ksim::filesystem::last_write_time(iter->path());
            
            if (now_time > file_modify_time && now_time - file_modify_time > sg_max_alive_time) {
                if(boost_ksim::filesystem::is_regular_file(iter->status())
                && iter->path().extension() == (std::string(".") + LOG_EXT)) {
                    boost_ksim::filesystem::remove(iter->path());
                } 
                if (boost_ksim::filesystem::is_directory(iter->status())) {
                    std::string filename = iter->path().filename().string();
                    if (filename.size() == 8 && filename.find_first_not_of("0123456789") == std::string::npos) {
                        boost_ksim::filesystem::remove_all(iter->path());
                    }
                }
            }
        }
...
static bool __append_file(const std::string& _src_file, const std::string& _dst_file) {
    ...
    if (!boost_ksim::filesystem::exists(_src_file)) {
        return false;
    }
    
    if (0 == boost_ksim::filesystem::file_size(_src_file)){
        return true;
    }
...
static void __move_old_files(const std::string& _src_path, const std::string& _dest_path, const std::string& _nameprefix) {
    ...
    boost_ksim::filesystem::path path(_src_path);
    if (!boost_ksim::filesystem::is_directory(path)) {
        return;
    }
...
static bool __cache_logs() {
    ...
    boost_ksim::filesystem::space_info info = boost_ksim::filesystem::space(sg_cache_logdir);
...
static void __log2file(const void* _data, size_t _len, bool _move_file) {
    ...
    if ((cache_logs || boost_ksim::filesystem::exists(logcachefilepath)) && __openlogfile(sg_cache_logdir)) {
        __writefile(_data, _len, sg_logfile);
        if (kAppednerAsync == sg_mode) {
            __closelogfile();
        }
        ...
        if (__append_file(logcachefilepath, logfilepath)) {
            ...
            boost_ksim::filesystem::remove(logcachefilepath);
        }
        return;
    }
...
const char* xlogger_dump(const void* _dumpbuffer, size_t _len) {
    ...
    if (!boost_ksim::filesystem::exists(filepath))
        boost_ksim::filesystem::create_directory(filepath);
...
void appender_open(TAppenderMode _mode, const char* _dir, const char* _nameprefix, const char* _pub_key) {
    ...
    boost_ksim::filesystem::create_directories(_dir);
    tickcount_t tick;
    tick.gettickcount();
    Thread(boost_ksim::bind(&__del_timeout_file, _dir)).start_after(2 * 60 * 1000);
    ...
    boost_ksim::filesystem::space_info info = boost_ksim::filesystem::space(sg_logdir);
...
void appender_open_with_cache(TAppenderMode _mode, const std::string& _cachedir, const std::string& _logdir,
    ...
    if (!_cachedir.empty()) {
        ...
        boost_ksim::filesystem::create_directories(_cachedir);

        Thread(boost_ksim::bind(&__del_timeout_file, _cachedir)).start_after(2 * 60 * 1000);
        // "_nameprefix" must explicitly convert to "std::string", or when the thread is ready to run, "_nameprefix" has been released.
        Thread(boost_ksim::bind(&__move_old_files, _cachedir, _logdir, std::string(_nameprefix))).start_after(3 * 60 * 1000);
    }
...
bool appender_make_logfile_name(int _timespan, const char* _prefix, std::vector<std::string>& _filepath_vec) {
    ...
    if (boost_ksim::filesystem::exists(log_path)) {
        _filepath_vec.push_back(log_path);
    }
    
    if (boost_ksim::filesystem::exists(cache_log_path)) {
        _filepath_vec.push_back(cache_log_path);
    }
    
    if (!boost_ksim::filesystem::exists(log_path) && !boost_ksim::filesystem::exists(cache_log_path)) {
        _filepath_vec.push_back(log_path);
    }
