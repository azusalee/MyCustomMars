...
SmartHeartbeatksim::SmartHeartbeatksim(): report_smart_heart_(NULL), is_wait_heart_response_(false), success_heart_count_(0), last_heart_(MinHeartInterval),
    ini_(marsksim::app::GetAppFilePath() + "/" + kFileName, false)
    , doze_mode_count_(0), normal_mode_count_(0), noop_start_tick_(false) {
    xinfo_function();
    ini_.Parse();
}

SmartHeartbeatksim::~SmartHeartbeatksim() {
    xinfo_function();
    __SaveINI();
}

void SmartHeartbeatksim::OnHeartbeatStart() {
    xverbose_function();
...
void SmartHeartbeatksim::OnLongLinkEstablished() {
    xdebug_function();
    __LoadINI();
    success_heart_count_ = 0;
}

void SmartHeartbeatksim::OnLongLinkDisconnect() {
    xinfo_function();
...
void SmartHeartbeatksim::OnHeartResult(bool _sucess, bool _fail_of_timeout) {
...
void SmartHeartbeatksim::JudgeDozeStyle() {
    
    if(ActiveLogicksim::Singletonksim::Instance()->IsActive())  return;
...
bool SmartHeartbeatksim::__IsDozeStyle() {
    return ((doze_mode_count_ > (2*normal_mode_count_)) && kMobile == ::getNetInfo());
}

unsigned int SmartHeartbeatksim::GetNextHeartbeatInterval() {  //
    // xinfo_function();
    
    if(ActiveLogicksim::Singletonksim::Instance()->IsActive()) {
        last_heart_ = MinHeartInterval;
...
void SmartHeartbeatksim::__LoadINI() {
    xinfo_function();
    std::string net_info;
...
void SmartHeartbeatksim::__LimitINISize() {
    xinfo_function();
...
void SmartHeartbeatksim::__SaveINI() {
    xdebug_function();
    if(current_net_heart_info_.net_detail_.empty())return;
...
void SmartHeartbeatksim::__DumpHeartInfo() {
    xinfo2(TSF"SmartHeartbeatksim Info last_heart_:%0,successHeartCount:%1, currSuccCount:%2", last_heart_, success_heart_count_, current_net_heart_info_.succ_heart_count_);
