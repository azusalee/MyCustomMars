...
#ifdef ANDROID
#include "marsksim/comm/android/wakeuplock.h"
#endif
...
#endif
{
    ...
    xinfo2(TSF"ShortLinkTaskManager messagequeue_id=%_", MessageQueueksim::Handler2Queue(asyncreg_.Get()));
}
...
#endif
        MessageQueueksim::FasterMessage(asyncreg_.Get(),
                                    MessageQueueksim::Message((MessageQueueksim::MessageTitle_t)this, boost_ksim::bind(&ShortLinkTaskManager::__RunLoop, this), "ShortLinkTaskManager::__RunLoop"),
                                    MessageQueueksim::MessageTiming(1000));
    } else {
#ifdef ANDROID
        /*cancel the last wakeuplock*/
        wakeup_lock_->Lock(500);
...
void ShortLinkTaskManager::__RunOnStartTask() {
    ...
    while (first != last) {
        ...
        ShortLinkInterface* worker = ShortLinkChannelFactory::Create(MessageQueueksim::Handler2Queue(asyncreg_.Get()), net_source_, first->task, first->use_proxy);
        worker->OnSend.set(boost_ksim::bind(&ShortLinkTaskManager::__OnSend, this, _1), AYNC_HANDLER);
        worker->OnRecv.set(boost_ksim::bind(&ShortLinkTaskManager::__OnRecv, this, _1, _2, _3), AYNC_HANDLER);
        worker->OnResponse.set(boost_ksim::bind(&ShortLinkTaskManager::__OnResponse, this, _1, _2, _3, _4, _5, _6, _7), AYNC_HANDLER);
...
void ShortLinkTaskManager::__OnRecv(ShortLinkInterface* _worker, unsigned int _cached_size, unsigned int _total_size) {
    ...
    if (lst_cmd_.end() != it) {
        if(it->transfer_profile.last_receive_pkg_time == 0)
            WeakNetworkLogic::Singletonksim::Instance()->OnPkgEvent(true, (int)(::gettickcount() - it->transfer_profile.start_send_time));
        else
            WeakNetworkLogic::Singletonksim::Instance()->OnPkgEvent(false, (int)(::gettickcount() - it->transfer_profile.last_receive_pkg_time));
        it->transfer_profile.last_receive_pkg_time = ::gettickcount();
...
bool ShortLinkTaskManager::__SingleRespHandle(std::list<TaskProfile>::iterator _it, ErrCmdType _err_type, int _err_code, int _fail_handle, size_t _resp_length, const ConnectProfile& _connect_profile) {
    ...
    if (_it->force_no_retry || 0 >= _it->remain_retry_count || kEctOK == _err_type || kTaskFailHandleTaskEnd == _fail_handle || kTaskFailHandleTaskTimeout == _fail_handle) {
        ...
        WeakNetworkLogic::Singletonksim::Instance()->OnTaskEvent(*_it);
...
void ShortLinkTaskManager::__DeleteShortLink(intptr_t& _running_id) {
    ...
    MessageQueueksim::CancelMessage(asyncreg_.Get(), p_shortlink);
    p_shortlink = NULL;
}
