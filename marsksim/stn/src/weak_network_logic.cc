...
namespace stn {
    ...
    WeakNetworkLogic::WeakNetworkLogic():is_curr_weak_(false), connect_after_weak_(0), last_connect_fail_tick_(false), last_connect_suc_tick_(false) {
        ActiveLogicksim::Singletonksim::Instance()->SignalForeground.connect(boost_ksim::bind(&WeakNetworkLogic::__SignalForeground, this, _1));
    }
    
    WeakNetworkLogic::~WeakNetworkLogic() {
        ActiveLogicksim::Singletonksim::Instance()->SignalForeground.disconnect(boost_ksim::bind(&WeakNetworkLogic::__SignalForeground, this, _1));
    }
    ...
    void WeakNetworkLogic::OnConnectEvent(bool _is_suc, int _rtt, int _index) {
        ...
        if(!ActiveLogicksim::Singletonksim::Instance()->IsForeground())
            return;
