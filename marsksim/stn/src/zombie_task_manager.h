...
class ZombieTaskManager {
  public:
    boost_ksim::function<void (const Task& _task)> fun_start_task_;
    boost_ksim::function<int (ErrCmdType _errtype, int _errcode, int _fail_handle, const Task& _task, unsigned int _taskcosttime)> fun_callback_;

  public:
    ZombieTaskManager(MessageQueueksim::MessageQueueksim_t _messagequeueid);
    ~ZombieTaskManager();
