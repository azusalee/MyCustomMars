...
LongLinkIdentifyCheckerksim::LongLinkIdentifyCheckerksim()
:has_checked_(false)
, cmd_id_(0)
, taskid_(0)
{ }
...
bool LongLinkIdentifyCheckerksim::GetIdentifyBuffer(AutoBuffer &_buffer, uint32_t &_cmdid)
{
...
void LongLinkIdentifyCheckerksim::SetID(uint32_t  _taskid) { taskid_ = _taskid;}

bool LongLinkIdentifyCheckerksim::IsIdentifyResp(uint32_t _cmdid, uint32_t _taskid, const AutoBuffer& _buffer, const AutoBuffer& _extend) const {
    return longlink_identify_isresp(taskid_, _cmdid, _taskid, _buffer, _extend);
}

bool LongLinkIdentifyCheckerksim::OnIdentifyResp(AutoBuffer& _buffer) {
...
void LongLinkIdentifyCheckerksim::Reset() {
    has_checked_ = false;
    taskid_ = 0;
    cmd_id_ = 0;
    hash_code_buffer_.Reset();
