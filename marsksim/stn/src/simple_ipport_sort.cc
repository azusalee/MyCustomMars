...
///////////////////////////////////////////////////////////////////////////////////////////
namespace marsksim { namespace stn {
    struct BanItem {
        std::string ip;
        uint16_t port;
        uint8_t records;
...
: hostpath_(marsksim::app::GetAppFilePath() + "/" + kFolderName) {
        
    if (!boost_ksim::filesystem::exists(hostpath_)){
        boost_ksim::filesystem::create_directory(hostpath_);
    }
...
void SimpleIPPortSort::__SortbyBanned(std::vector<IPPortItem>& _items) const {
    ...
    items_history.erase(std::remove_copy_if(_items.begin(), _items.end(), items_history.begin(), !boost_ksim::bind<bool>(find_lambda, _1)), items_history.end());
    ...
    std::sort(items_history.begin(), items_history.end(),
              [&](const IPPortItem& _l, const IPPortItem& _r){
                  ...
                  std::vector<BanItem>::const_iterator  l = std::find_if(_ban_fail_list_.begin(), _ban_fail_list_.end(), boost_ksim::bind<bool>(find_lr_lambda, _1, _l));
                  std::vector<BanItem>::const_iterator  r = std::find_if(_ban_fail_list_.begin(), _ban_fail_list_.end(), boost_ksim::bind<bool>(find_lr_lambda, _1, _r));
