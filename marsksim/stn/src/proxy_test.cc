...
bool ProxyTest::ProxyIsAvailable(const marsksim::comm::ProxyInfo _proxy_info, const std::string& _host, const std::vector<std::string>& _hardcode_ips) {
...
SOCKET ProxyTest::__Connect(const marsksim::comm::ProxyInfo& _proxy_info, const std::string& _host, const std::vector<std::string>& _hardcode_ips) {
    NetSourceksim::DnsUtil dns_util_;
    
    std::string proxy_ip;
    if (marsksim::comm::kProxyNone != _proxy_info.type) {
    ...
    if (marsksim::comm::kProxyHttp == _proxy_info.type) {
    ...
    if (marsksim::comm::kProxyNone != _proxy_info.type && marsksim::comm::kProxyHttp != _proxy_info.type) {
...
int ProxyTest::__ReadWrite(SOCKET _sock, const marsksim::comm::ProxyInfo& _proxy_info, const std::string& _host) {
    
    std::map<std::string, std::string> headers;
    headers[http::HeaderFields::KStringHost] = _host;
    
    if (_proxy_info.IsValid() && marsksim::comm::kProxyHttp == _proxy_info.type && !_proxy_info.username.empty() && !_proxy_info.password.empty()) {
    ...
    if (marsksim::comm::kProxyHttp == _proxy_info.type && !_proxy_info.username.empty() && !_proxy_info.password.empty()) {
        std::string account_info = _proxy_info.username + ":" + _proxy_info.password;
        size_t dstlen = modp_b64_encode_len(account_info.length());
