...
#include "marsksim/stn/dns_profile.h"
...
NetSourceksim::DnsUtil::DnsUtil():
new_dns_(OnNewDns) {
}

NetSourceksim::DnsUtil::~DnsUtil() {}

void NetSourceksim::DnsUtil::Cancel(const std::string& host) {
    if (host.empty()) {
        new_dns_.Cancel();
        dns_.Cancel();
    }
...
NetSourceksim::NetSourceksim(ActiveLogicksim& _active_logic)
	: active_logic_(_active_logic)
{
    xinfo_function();
}

NetSourceksim::~NetSourceksim() {
    xinfo_function();
}
 ...
 */
void NetSourceksim::SetLongLink(const std::vector<std::string>& _hosts, const std::vector<uint16_t>& _ports, const std::string& _debugip) {
...
void NetSourceksim::SetShortlink(const uint16_t _port, const std::string& _debugip) {
	ScopedLock lock(sg_ip_mutex);
...
void NetSourceksim::SetBackupIPs(const std::string& _host, const std::vector<std::string>& _ips) {
	ScopedLock lock(sg_ip_mutex);
...
void NetSourceksim::SetDebugIP(const std::string& _host, const std::string& _ip) {
	ScopedLock lock(sg_ip_mutex);
...
const std::string& NetSourceksim::GetLongLinkDebugIP() {
	ScopedLock lock(sg_ip_mutex);

	return sg_longlink_debugip;
}

const std::string& NetSourceksim::GetShortLinkDebugIP() {
    ScopedLock lock(sg_ip_mutex);
    return sg_shortlink_debugip;
}

void NetSourceksim::SetLowPriorityLonglinkPorts(const std::vector<uint16_t>& _lowpriority_longlink_ports) {
 ...
 *
 */
const std::vector<std::string>& NetSourceksim::GetLongLinkHosts() {
    ScopedLock lock(sg_ip_mutex);
	return sg_longlink_hosts;
}

void NetSourceksim::GetLonglinkPorts(std::vector<uint16_t>& _ports) {
    ScopedLock lock(sg_ip_mutex);
	_ports = sg_longlink_ports;
}

bool NetSourceksim::GetLongLinkItems(std::vector<IPPortItem>& _ipport_items, DnsUtil& _dns_util) {
    xinfo_function();
    ScopedLock lock(sg_ip_mutex);
    ...
    lock.unlock();
 	...
 	std::vector<std::string> longlink_hosts = NetSourceksim::GetLongLinkHosts();
...
bool NetSourceksim::__GetLonglinkDebugIPPort(std::vector<IPPortItem>& _ipport_items) {
...
void NetSourceksim::GetBackupIPs(std::string _host, std::vector<std::string>& _iplist) {
	ScopedLock lock(sg_ip_mutex);
...
void NetSourceksim::ReportLongIP(bool _is_success, const std::string& _ip, uint16_t _port) {
...
void NetSourceksim::RemoveLongBanIP(const std::string& _ip) {
 ...
 *
 */
uint16_t NetSourceksim::GetShortLinkPort() {
	ScopedLock lock(sg_ip_mutex);
	return sg_shortlink_port;
}

bool NetSourceksim::__HasShortLinkDebugIP(const std::vector<std::string>& _hostlist) {
...
bool NetSourceksim::GetShortLinkItems(const std::vector<std::string>& _hostlist, std::vector<IPPortItem>& _ipport_items, DnsUtil& _dns_util) {
...
bool NetSourceksim::__GetShortlinkDebugIPPort(const std::vector<std::string>& _hostlist, std::vector<IPPortItem>& _ipport_items) {
...
void NetSourceksim::__GetIPPortItems(std::vector<IPPortItem>& _ipport_items, const std::vector<std::string>& _hostlist, DnsUtil& _dns_util, bool _islonglink) {
...
size_t NetSourceksim::__MakeIPPorts(std::vector<IPPortItem>& _ip_items, const std::string& _host, size_t _count, DnsUtil& _dns_util, bool _isbackup, bool _islonglink) {
	...
	if (!_isbackup) {
		...
		if (_islonglink) {
			NetSourceksim::GetLonglinkPorts(ports);
		}
		else {
			ports.push_back(NetSourceksim::GetShortLinkPort());
		}
	}
	else {
		NetSourceksim::GetBackupIPs(_host, iplist);
...
void NetSourceksim::ReportShortIP(bool _is_success, const std::string& _ip, const std::string& _host, uint16_t _port) {
...
void NetSourceksim::ClearCache() {
    xinfo_function();
    ipportstrategy_.InitHistory2BannedList(true);
}

std::string NetSourceksim::DumpTable(const std::vector<IPPortItem>& _ipport_items) {
    XMessage stream;
...
bool NetSourceksim::GetLongLinkSpeedTestIPs(std::vector<IPPortItem>& _ip_vec) {
    xverbose_function();

    return true;
}

void NetSourceksim::ReportLongLinkSpeedTestResult(std::vector<IPPortItem>& _ip_vec) {
}

void NetSourceksim::AddServerBan(const std::string& _ip) {
