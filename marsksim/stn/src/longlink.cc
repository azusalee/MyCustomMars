...
#include "marsksim/comm/socket/getsocktcpinfo.h"
#endif
...
namespace {
class LongLinkConnectObserverksim : public MComplexConnectksim {
  public:
    LongLinkConnectObserverksim(LongLink& _longlink, const std::vector<IPPortItem>& _iplist): longlink_(_longlink), ip_items_(_iplist) {
  ...
  private:
    LongLinkConnectObserverksim(const LongLinkConnectObserverksim&);
    LongLinkConnectObserverksim& operator=(const LongLinkConnectObserverksim&);
...
#ifdef ANDROID
    , smartheartbeat_(new SmartHeartbeatksim)
...
bool LongLink::__NoopReq(XLogger& _log, Alarm& _alarm, bool need_active_timeout) {
    ...
    if (identifychecker_.GetIdentifyBuffer(buffer, req_cmdid)) {
        Task task(Task::kLongLinkIdentifyCheckerksimTaskID);
        task.cmdid = req_cmdid;
        suc = Send(buffer, KNullAtuoBuffer, task);
        identifychecker_.SetID(Task::kLongLinkIdentifyCheckerksimTaskID);
        xinfo2(TSF"start noop synccheck taskid:%0, cmdid:%1, ", Task::kLongLinkIdentifyCheckerksimTaskID, req_cmdid) >> _log;
    } else {
...
bool LongLink::__NoopResp(uint32_t _cmdid, uint32_t _taskid, AutoBuffer& _buf, AutoBuffer& _extension, Alarm& _alarm, bool& _nooping, ConnectProfile& _profile) {
    ...
    if (is_noop && _nooping) {
        _nooping = false;
        _alarm.Cancel();
        __NotifySmartHeartbeatksimHeartResult(true, false, _profile);
#ifdef ANDROID
        wakelock_->Lock(500);
#endif
    }
...
SOCKET LongLink::__RunConnect(ConnectProfile& _conn_profile) {
    ...
    marsksim::comm::ProxyInfo proxy_info = marsksim::app::GetProxyInfo("");
    bool use_proxy = proxy_info.IsValid() && marsksim::comm::kProxyNone != proxy_info.type && marsksim::comm::kProxyHttp != proxy_info.type && netsource_.GetLongLinkDebugIP().empty();
    xinfo2(TSF"task socket dns ip:%_ proxytype:%_ useproxy:%_", NetSourceksim::DumpTable(ip_items), proxy_info.type, use_proxy);
    ...
    LongLinkConnectObserverksim connect_observer(*this, ip_items);
    ComplexConnectksim com_connect(kLonglinkConnTimeout, kLonglinkConnInteral, kLonglinkConnInteral, kLonglinkConnMax);
    ...
    while (true) {
        if (!alarmnoopinterval.IsWaiting()) {
            ...
            if (__NoopReq(noop_xlog, alarmnooptimeout, has_late_toomuch)) {
                nooping = true;
                __NotifySmartHeartbeatksimHeartReq(_profile, last_noop_interval, last_noop_actual_interval);
            }
...
//            __NotifySmartHeartbeatksimJudgeDozeStyle();
        ...
        if (sel.Write_FD_ISSET(_sock) && !lstsenddata_.empty()) {
            ...
            GetSignalksimOnNetworkDataChange()(XLOGGER_TAG, writelen, 0);
        ...
        if (sel.Read_FD_ISSET(_sock)) {
            ...
            GetSignalksimOnNetworkDataChange()(XLOGGER_TAG, 0, recvlen);
...
End:
    if (nooping) __NotifySmartHeartbeatksimHeartResult(false, (_errcode == kEctSocketRecvErr), _profile);
...
void LongLink::__NotifySmartHeartbeatksimHeartReq(ConnectProfile& _profile, uint64_t _internal, uint64_t _actual_internal) {
...
void LongLink::__NotifySmartHeartbeatksimHeartResult(bool _succes, bool _fail_of_timeout, ConnectProfile& _profile) {
    if (longlink_noop_interval() > 0) {
        return;
    }
...
void LongLink::__NotifySmartHeartbeatksimJudgeDozeStyle() {
    if (longlink_noop_interval() > 0) {
        return;
    }
...
void LongLink::__NotifySmartHeartbeatksimConnectStatus(TLongLinkStatus _status) {
    if (longlink_noop_interval() > 0) {
        return;
    }
