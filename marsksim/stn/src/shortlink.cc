...
#include "marsksim/comm/socket/block_socket.h"
...
#include "marsksim/comm/crypt/ibase64.h"
...
#endif
#include "marsksim/stn/proto/shortlink_packer.h"
...
class ShortLinkConnectObserver : public MComplexConnectksim {
  public:
...
ShortLink::ShortLink(MessageQueueksim::MessageQueueksim_t _messagequeueid, NetSourceksim& _netsource, const Task& _task, bool _use_proxy)
    : asyncreg_(MessageQueueksim::InstallAsyncHandler(_messagequeueid))
	, net_source_(_netsource)
	, task_(_task)
	, thread_(boost_ksim::bind(&ShortLink::__Run, this), XLOGGER_TAG "::shortlink")
...
SOCKET ShortLink::__RunConnect(ConnectProfile& _conn_profile) {
    ...
    if (use_proxy_) {
        _conn_profile.proxy_info = marsksim::app::GetProxyInfo(_conn_profile.host);
    }
    
    bool use_proxy = use_proxy_ && _conn_profile.proxy_info.IsValid();
    bool isnat64 = ELocalIPStack_IPv6 == local_ipstack_detect();

    if (use_proxy && marsksim::comm::kProxyHttp == _conn_profile.proxy_info.type && net_source_.GetShortLinkDebugIP().empty()) {
	...
	std::string proxy_ip;
    if (use_proxy && marsksim::comm::kProxyNone != _conn_profile.proxy_info.type) {
    ...
    if (use_proxy && marsksim::comm::kProxyHttp == _conn_profile.proxy_info.type) {
    ...
    if (use_proxy && (marsksim::comm::kProxyHttpTunel == _conn_profile.proxy_info.type || marsksim::comm::kProxySocks5 == _conn_profile.proxy_info.type)) {
		proxy_addr = &((new socket_address(proxy_ip.c_str(), _conn_profile.proxy_info.port))->v4tov6_address(isnat64));
        _conn_profile.ip_type = kIPSourceProxy;
    }

    xinfo2(TSF"task socket dns sock %_ proxy:%_, host:%_, ip list:%_", message.String(), kIPSourceProxy == _conn_profile.ip_type, _conn_profile.host, NetSourceksim::DumpTable(_conn_profile.ip_items));
	...
	ComplexConnectksim conn(kShortlinkConnTimeout, kShortlinkConnInterval);
    ...
    WeakNetworkLogic::Singletonksim::Instance()->OnConnectEvent(sock!=INVALID_SOCKET, conn.IndexRtt(), conn.Index());
...
void ShortLink::__RunReadWrite(SOCKET _socket, int& _err_type, int& _err_code, ConnectProfile& _conn_profile) {
	...
	if (_conn_profile.proxy_info.IsValid() && marsksim::comm::kProxyHttp == _conn_profile.proxy_info.type
    ...
    GetSignalksimOnNetworkDataChange()(XLOGGER_TAG, send_ret, 0);
	...
	while (true) {
		...
		if (recv_ret > 0) {
            GetSignalksimOnNetworkDataChange()(XLOGGER_TAG, 0, recv_ret);
