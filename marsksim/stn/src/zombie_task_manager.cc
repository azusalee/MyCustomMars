...
ZombieTaskManager::ZombieTaskManager(MessageQueueksim::MessageQueueksim_t _messagequeueid)
...
bool ZombieTaskManager::SaveTask(const Task& _task, unsigned int _taskcosttime)
{
    ...
    MessageQueueksim::SingletonksimMessage(false, asyncreg_.Get(),
                                    MessageQueueksim::Message((MessageQueueksim::MessageTitle_t)this,
                                    boost_ksim::bind(&ZombieTaskManager::__TimerChecker, this), "ZombieTaskManager::__TimerChecker"),
                                    MessageQueueksim::MessageTiming(3000, 3000));
    return true;
}
...
void ZombieTaskManager::OnNetCoreksimStartTask()
{
    net_core_last_start_task_time_ = gettickcount();
}
...
void ZombieTaskManager::__TimerChecker()
{
    ...
    if (lsttask.empty()) MessageQueueksim::CancelMessage(asyncreg_.Get(), (MessageQueueksim::MessageTitle_t)this);
}
