...
#include "../../../network/MMNetSourceksimTimerCheck.h"
...
class TestCostItem
{
  public:
	...
	TestCostItem()
	{
		...
		connectHardcodeIPcost = 6*4*1000 + 500;//6 ComplexConnectksim intervals
...
{
	return EIPSourceNewDns == CMMNetCoreksim::Singletonksim()->m_longLinkTaskManager->LongLinkChannel().m_connInfo.ipType;
}
static bool isLonglinkHardCodeIP()
{
	return EIPSourceHardcode == CMMNetCoreksim::Singletonksim()->m_longLinkTaskManager->LongLinkChannel().m_connInfo.ipType;
}
static bool isSatelliteIdcNewdnsIP()
{
	bool ret = true;
	if(EIPSourceNewDns != CMMNetCoreksim::Singletonksim()->m_longLinkTaskManager->LongLinkChannel().m_connInfo.ipType)
	{
		ret = false;
	}
	const char *strIP = CMMNetCoreksim::Singletonksim()->m_longLinkTaskManager->LongLinkChannel().m_connInfo.ip.c_str();
...
static void printLonglinkConnInfo()
{
	...
	LongLinkConnectionInfo connInfo = CMMNetCoreksim::Singletonksim()->m_longLinkTaskManager->LongLinkChannel().m_connInfo;
...
bool CDetour::Mine_TryConnect(const string& _host)
{
	...
	if(3 == sg_TestType&&sg_readyToSwitchBackToSatelliteIdc)
	{
		...
		{
			CMMNetCoreksim::Singletonksim()->m_netsource->RemoveLongBanIP(iter->strIP);
...
bool (CDetour::* CDetour::Real_GetLongLinkItems)(std::vector<IPPortItem>& _IPPortItems) = (bool (CDetour::*)(std::vector<IPPortItem>& _IPPortItems)) &CMMNetSourceksim::GetLongLinkItems;
bool (CDetour::* CDetour::Real_GetMixLongLinkItems)(std::vector<IPPortItem>& _IPPortItems) = (bool (CDetour::*)(std::vector<IPPortItem>& _IPPortItems)) &CMMNetSourceksim::GetMixLongLinkItems;
void (CDetour::* CDetour::Real_ReportLongIP)(bool _isSuccess, const std::string& _ip, unsigned int _port) = (void (CDetour::*)(bool _isSuccess, const std::string& _ip, unsigned int _port)) &CMMNetSourceksim::ReportLongIP;
//4:
...
void * p3 = (void *)DetourFindFunction("test.exe","CMMNetSourceksimTimerCheck::__TryConnnect");
...
{
	_netcore->m_longLinkTaskManager->funCallback = boost_ksim::bind(&CallBack, 0, _1, _2, _3, _4, _5, _6);
...
static void funcSwitchIpTest(TestCaseInfo *m_testCaseInfo, const WholeTestParam& param)
{
	...
	if(param.isForeGround) SINGLETON_STRONG_ksim(ActiveLogicksim).m_lastforegroundchangetime -= param.timeAccelerateMilliSec;
...
PUBC_TEST(MMWholeTest, test0)
{
	...
	for(int index=0; index < EACH_SITUATION_RUN_COUNT; ++index)
	{
		...
		param.expectMaxCostTime.connectHardcodeIPcost = (24)*1000;//ComplexConnectksim time cost�� hardcodeIP is 6th ip ��1 host 2 ports��
...
PUBC_TEST(MMWholeTest, test10)
{
	...
	for(int index=0; index < EACH_SITUATION_RUN_COUNT; ++index)
	{
		...
		param.expectMaxCostTime.ConnectSatelliteNewdnsIpCost = 5*1000;
		param.expectMaxCostTime.ConnectMainNewdnsIpCost = 5*60*1000 + 24*1000; //24 stands for ComplexConnectksim time cost�� 5min stands for (EBackgroundActive state, the interval of autointerval)
