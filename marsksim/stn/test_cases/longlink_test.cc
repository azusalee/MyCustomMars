...
void test(TestCaseInfo& _info)
{
	...
	longlink.SignalConnection.connect(boost_ksim::bind(&OnLongLinkStatuChanged, _1));

	longlink.FuncNetworkReport = boost_ksim::bind(&__OnLongLinkNetworkError, _1, _2, _3, _4, _5);;
	...
	for (size_t i=0;i<sg_longLinkScenes.at(sg_index).expectValues.size();++i)
	{
		...
		ThreadUtilksim::sleep(expectValue.first);
