...
#include "../../../network/MMNetCoreksim.h"
...
#include "../../../mmcomm/ActiveLogicksim.h"
...
static void banAllNewdnsIP()
{
	...
	if(!isAlreadyBanned)
	{
		for(int i=0; i<3; ++i)
		{
			CMMNetCoreksim::Singletonksim()->GetNetSourceksim().ReportShortIP(false, "127.0.50.1", "short.weixin.qq.com");
			CMMNetCoreksim::Singletonksim()->GetNetSourceksim().ReportShortIP(false, "127.0.50.2", "short.weixin.qq.com");
			CMMNetCoreksim::Singletonksim()->GetNetSourceksim().ReportShortIP(false, "127.0.50.3", "short.weixin.qq.com");

			CMMNetCoreksim::Singletonksim()->GetNetSourceksim().ReportShortIP(false, "127.0.51.1", "sz2tjextshort.weixin.qq.com");
			CMMNetCoreksim::Singletonksim()->GetNetSourceksim().ReportShortIP(false, "127.0.51.2", "sz2tjextshort.weixin.qq.com");
			CMMNetCoreksim::Singletonksim()->GetNetSourceksim().ReportShortIP(false, "127.0.51.3", "sz2tjextshort.weixin.qq.com");
		}
...
DECLARE_HOOK_BEGIN
	DECLARE_HOOK_FUNCTION3(CMMNetSourceksim, GetShortLinkProxyInfo, bool, unsigned int&, std::string&, std::string&);
	DECLARE_HOOK_FUNCTION2(CMMNetSourceksim, GetShortLinkIPs, bool, const std::vector<std::string>&, std::vector<IPPortItem>&);
	DECLARE_HOOK_FUNCTION3(CMMNetSourceksim, ReportShortIP, void, bool, const std::string&, const std::string&);
...
ATTACH_HOOK_BEGIN
	ATTACH_HOOK_FUNCTION3(CMMNetSourceksim, GetShortLinkProxyInfo, bool, unsigned int&, std::string&, std::string&);
	ATTACH_HOOK_FUNCTION2(CMMNetSourceksim, GetShortLinkIPs, bool, const std::vector<std::string>&, std::vector<IPPortItem>&);
	ATTACH_HOOK_FUNCTION3(CMMNetSourceksim, ReportShortIP, void, bool, const std::string&, const std::string&);
...
DETACH_HOOK_BEGIN
	DETACH_HOOK_FUNCTION3(CMMNetSourceksim, GetShortLinkProxyInfo, bool, unsigned int&, std::string&, std::string&);
	DETACH_HOOK_FUNCTION2(CMMNetSourceksim, GetShortLinkIPs, bool, const std::vector<std::string>&, std::vector<IPPortItem>&);
	ATTACH_HOOK_FUNCTION3(CMMNetSourceksim, ReportShortIP, void,  bool, const std::string&, const std::string&);
...
IMPLEMENT_MINE_HOOK_FUNCITON3(CMMNetSourceksim, GetShortLinkProxyInfo, bool, unsigned int&, port, std::string&, strProxy, std::string&, _host)
{
	(this->*Real_GetShortLinkProxyInfo)(port, strProxy, _host);
	return false;
}

IMPLEMENT_MINE_HOOK_FUNCITON3(CMMNetSourceksim, ReportShortIP, void, bool, _isSuccess, const std::string&, _ip, const std::string&, _host)
{
	(this->*Real_ReportShortIP)(_isSuccess, _ip, _host);
	printf("******connect %s: ip=%s, host=%s************************\n", _isSuccess?"success":"failed", _ip.c_str(), _host.c_str());
}

IMPLEMENT_MINE_HOOK_FUNCITON2(CMMNetSourceksim, GetShortLinkIPs, bool, const std::vector<std::string>&, _hostlist, std::vector<IPPortItem>&, _IPPortItems)
{
	if(1==sg_TestType)
	{
		banAllNewdnsIP();
...
SOCKET CDetour::Mine___MakeSocketPrepared()
{
	if(!sg_isEnd)
	{
		...
		{
			std::list<STCmdShortLinkTask> temp_lstCmd = CMMNetCoreksim::Singletonksim()->m_shortLinkTaskManager->m_lstCmd;
...
static void ChangeBindFunc(CMMNetCoreksim* _netcore)
{
	_netcore->m_shortLinkTaskManager->funCallback = boost_ksim::bind(&CallBack, 0, _1, _2, _3, _4, _5, _6);
...
static void shortIpSwitchTest(TestCaseInfo *m_testCaseInfo, const IpSwitchParam& param)
{
	...
	SINGLETON_STRONG_ksim(ActiveLogicksim).OnForeground(param.isForeGround);
	SINGLETON_STRONG_ksim(ActiveLogicksim).m_isactive = true;
	UtilFunc::del_files(CONFIG_PATH);
	xlogger_SetLevel(ELevelDebug);
	appender_open(EAppednerSync, "C:\\log", "GTEST");
	Hook();
	HookPrivate();
	ChangeBindFunc(CMMNetCoreksim::Singletonksim());
	...
	if(param.idcHostTable.empty())
	{
		...
		CMMNetCoreksim::Singletonksim()->GetNetSourceksim().GetHostRedirect().SaveIDCHostMapping(idchostTable);
	}
	else
	{
		CMMNetCoreksim::Singletonksim()->GetNetSourceksim().GetHostRedirect().SaveIDCHostMapping(param.idcHostTable);
	}
	...
	for (int i=0;i<1000;++i)
	{
		...
		{
			CMMNetCoreksim::Singletonksim()->StartTask(9527+i, netcmd);
