...
#include "marsksim/stn/stn_logic.h"
...
#include "marsksim/boost/signals2.hpp"
...
boost_ksim::signals2::signal<void (ErrCmdType _err_type, int _err_code, const std::string& _ip, uint16_t _port)> SignalOnLongLinkNetworkError;
boost_ksim::signals2::signal<void (ErrCmdType _err_type, int _err_code, const std::string& _ip, const std::string& _host, uint16_t _port)> SignalOnShortLinkNetworkError;

#define STN_WEAK_CALL_ksim(func) \
    boost_ksim::shared_ptr<NetCoreksim> stn_ptr = NetCoreksim::Singletonksim::Instance_Weak().lock();\
    if (!stn_ptr) {\
...
#define STN_RETURN_WEAK_CALL_ksim(func) \
    boost_ksim::shared_ptr<NetCoreksim> stn_ptr = NetCoreksim::Singletonksim::Instance_Weak().lock();\
    if (!stn_ptr) {\
        xwarn2(TSF"stn uncreate");\
        return false;\
...
#define STN_WEAK_CALL_ksim_RETURN(func, ret) \
	boost_ksim::shared_ptr<NetCoreksim> stn_ptr = NetCoreksim::Singletonksim::Instance_Weak().lock();\
    if (stn_ptr) \
    {\
    	ret = stn_ptr->func;\
...
static void onCreate() {
    ...
    ActiveLogicksim::Singletonksim::Instance();
    NetCoreksim::Singletonksim::Instance();
...
static void onNetworkChange() {
    ...
    STN_WEAK_CALL_ksim(OnNetworkChange());
}
...
static void __initbind_baseprjevent() {
...
#ifdef WIN32
	boost_ksim::filesystem::path::imbue(std::locale(std::locale(), new boost_ksim::filesystem::detail::utf8_codecvt_facet));
#endif
    ...
    GetSignalksimOnDestroy().connect(&onDestroy);   //low priority signal func
    GetSignalksimOnSingalCrash().connect(&onSingalCrash);
    GetSignalksimOnExceptionCrash().connect(&onExceptionCrash);
    GetSignalksimOnNetworkChange().connect(5, &onNetworkChange);    //define group 5
...
#error "not define XLOGGER_TAG"
    ...
    GetSignalksimOnNetworkDataChange().connect(&OnNetworkDataChange);
}
...
= [](const Task& _task) {
    STN_RETURN_WEAK_CALL_ksim(StartTask(_task));
};

void (*StopTask)(uint32_t _taskid)
= [](uint32_t _taskid) {
    STN_WEAK_CALL_ksim(StopTask(_taskid));
};

bool (*HasTask)(uint32_t _taskid)
= [](uint32_t _taskid) {
	bool has_task = false;
	STN_WEAK_CALL_ksim_RETURN(HasTask(_taskid), has_task);
	return has_task;
};

void (*RedoTasks)()
= []() {
   STN_WEAK_CALL_ksim(RedoTasks());
};

void (*ClearTasks)()
= []() {
   STN_WEAK_CALL_ksim(ClearTasks());
};
...
= []() {
   ...
   STN_WEAK_CALL_ksim(MakeSureLongLinkConnect());
};
    
void (*DisConnectLonglink)()
= []() {
    STN_WEAK_CALL_ksim(DisConnectLonglink());
};

bool (*LongLinkIsConnected)()
= []() {
    bool connected = false;
    STN_WEAK_CALL_ksim_RETURN(LongLinkIsConnected(), connected);
    return connected;
};
    
bool (*ProxyIsAvailable)(const marsksim::comm::ProxyInfo& _proxy_info, const std::string& _test_host, const std::vector<std::string>& _hardcode_ips)
= [](const marsksim::comm::ProxyInfo& _proxy_info, const std::string& _test_host, const std::vector<std::string>& _hardcode_ips){
    
    return ProxyTest::Singletonksim::Instance()->ProxyIsAvailable(_proxy_info, _test_host, _hardcode_ips);
};
...
= [](const std::string& host, const std::vector<std::string>& iplist) {
	NetSourceksim::SetBackupIPs(host, iplist);
};
