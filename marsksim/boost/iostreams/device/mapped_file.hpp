...
// mars_boost::filesystem::path, mars_boost::filesystem::wpath, 
// and mars_boost::iostreams::detail::path (used to store either a std::string or a
// std::wstring).
...
private:
    ...
    friend struct mars_boost::iostreams::operations<mapped_file_source>;
public:
...
private:
    ...
    mars_boost::shared_ptr<impl_type> pimpl_;
};
...
mapped_file_source::mapped_file_source( 
    const Path& path, size_type length, mars_boost::intmax_t offset)
...
mapped_file_sink::mapped_file_sink(
    ...
    mars_boost::intmax_t offset, mapmode flags )
...
struct operations<mapped_file_source>
    : mars_boost::iostreams::detail::close_impl<closable_tag>
{
