...
template <class T> struct access_traits {
  ...
  typedef const typename mars_boost::remove_cv<T>::type& parameter_type;
...
template <class T> struct wrap_non_storeable_type {
  typedef typename IF<
    ::mars_boost::is_function<T>::value, non_storeable_type<T>, T
  >::RET type;
};
...
inline void swap(tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>& lhs,
  ...
  ::mars_boost::tuples::swap(static_cast<base&>(lhs), static_cast<base&>(rhs));
}
