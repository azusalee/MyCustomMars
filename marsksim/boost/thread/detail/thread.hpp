...
//            {}
        ...
        template<typename F>
        class thread_data<boost_ksim::reference_wrapper<F> >:
            public detail::thread_data_base
        {
        private:
            F& f;
    ...
    public:
        ...
        template <class F>
        thread(attributes const& attrs, F f
            , typename disable_if<boost_ksim::thread_detail::is_rv<F>, dummy* >::type=0
            //, typename disable_if<boost_ksim::thread_detail::is_convertible<F&,BOOST_THREAD_RV_REF(F) >, dummy* >::type=0
        ):
            thread_info(make_thread_info(f))
        {
            start_thread(attrs);
