...
namespace mars_boost_ksim {} namespace boost_ksim = mars_boost_ksim; namespace mars_boost_ksim
{
  namespace detail
  {
    ...
    {
      mars_boost_ksim::unique_lock<MutexType1> l1(m1, mars_boost_ksim::try_to_lock);
      if (!l1)
      {
        return 1;
      }
    ...
    {
      mars_boost_ksim::unique_lock<MutexType1> l1(m1);
      if (!m2.try_lock())
      {
        return 1;
      }
  ...
  namespace detail
  {
    ...
    struct range_lock_guard
    {
      ...
      {
        mars_boost_ksim::lock(begin, end);
      }
...

    {
      ...
      Iterator const failed = mars_boost_ksim::try_lock(++begin, end);
      if (failed == end)
      {
        guard.release();
      }
  ...
  namespace detail
  {
    ...
    void lock_impl(Iterator begin, Iterator end, is_mutex_type_wrapper<false> )
    {
      ...
      for (;;)
      {
        ...
        {
          begin_lock.lock();
          Iterator const failed_lock = mars_boost_ksim::try_lock(next, end);
          if (failed_lock == end)
          {
            begin_lock.release();
            return;
        ...
        else
        {
          ...
          {
            Iterator const failed_lock = mars_boost_ksim::try_lock(second, next);
            if (failed_lock == next)
            {
              begin_lock.release();
              guard.release();
