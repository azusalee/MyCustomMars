...
namespace mars_boost_ksim {} namespace boost_ksim = mars_boost_ksim; namespace mars_boost_ksim
{
  ...
  public:
    ...
    {
      unique_lock temp(::boost_ksim::move(other));
      swap(temp);
      return *this;
    }
#endif
    ...
    bool timed_lock(::boost_ksim::system_time const& absolute_time)
    {
      if(m==0)
      {
        mars_boost_ksim::throw_exception(mars_boost_ksim::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
    ...
    bool timed_lock(::boost_ksim::xtime const& absolute_time)
    {
      if(m==0)
      {
        mars_boost_ksim::throw_exception(mars_boost_ksim::lock_error(static_cast<int>(system::errc::operation_not_permitted), "boost unique_lock has no mutex"));
  ...
  public:
    ...
    {
      shared_lock temp(::boost_ksim::move(other));
      swap(temp);
      return *this;
    }
#ifndef BOOST_THREAD_PROVIDES_EXPLICIT_LOCK_CONVERSION
  ...
  public:
    ...
    {
      upgrade_lock temp(::boost_ksim::move(other));
      swap(temp);
      return *this;
    }
  ...
  public:
    ...
    explicit upgrade_to_unique_lock(upgrade_lock<Mutex>& m_) :
      source(&m_), exclusive(::boost_ksim::move(*source))
    {
    }
    ~upgrade_to_unique_lock()
    {
      if (source)
      {
        *source = BOOST_THREAD_MAKE_RV_REF(upgrade_lock<Mutex> (::boost_ksim::move(exclusive)));
      }
    }

    upgrade_to_unique_lock(BOOST_THREAD_RV_REF_BEG upgrade_to_unique_lock<Mutex> BOOST_THREAD_RV_REF_END other) BOOST_NOEXCEPT:
    source(BOOST_THREAD_RV(other).source),exclusive(::boost_ksim::move(BOOST_THREAD_RV(other).exclusive))
    {
...
private unique_lock<Mutex>
  ...
  public:
    ...
    base(::boost_ksim::move(other))
    {}

#elif defined BOOST_THREAD_USES_MOVE
    try_lock_wrapper(BOOST_THREAD_RV_REF(try_lock_wrapper) other):
    base(::boost_ksim::move(static_cast<base&>(other)))
    {}
