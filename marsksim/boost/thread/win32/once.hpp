...
#define BOOST_THREAD_INVOKE_RET_VOID_CALL ()
    ...
    namespace detail
    {
...
#ifdef BOOST_NO_ANSI_APIS
            return ::boost_ksim::detail::win32::OpenEventW(
#else
            return ::boost_ksim::detail::win32::OpenEventA(
#endif
                ::boost_ksim::detail::win32::synchronize |
                ::boost_ksim::detail::win32::event_modify_state,
                false,
                mutex_name);
        }
        ...
        inline void* create_once_event(once_char_type* mutex_name,void* flag_address)
        {
            ...
            return ::boost_ksim::detail::win32::create_event(
                mutex_name, 
                ::boost_ksim::detail::win32::manual_reset_event,
                ::boost_ksim::detail::win32::event_initially_reset);
        }
        ...
        inline bool enter_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
          ...
          if(!status)
          {
            ...
            if(ctx.event_handle)
            {
                ::boost_ksim::detail::win32::ResetEvent(ctx.event_handle);
            }
            return true;
          }
          return false;
        ...
        inline void commit_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
          ...
          if(!ctx.event_handle &&
             (::boost_ksim::detail::interlocked_read_acquire(&flag.count)>1))
          {
              ctx.event_handle=detail::create_once_event(ctx.mutex_name,&flag);
          }
          if(ctx.event_handle)
          {
              ::boost_ksim::detail::win32::SetEvent(ctx.event_handle);
          }
        }
        inline void rollback_once_region(once_flag& flag, once_context& ctx) BOOST_NOEXCEPT
        {
...
//#if defined(BOOST_THREAD_RVALUE_REFERENCES_DONT_MATCH_FUNTION_PTR)
    inline void call_once(once_flag& flag, void (*f)())
    {
        ...
        while(::boost_ksim::detail::interlocked_read_acquire(&flag.status)
              !=ctx.function_complete_flag_value)
        {
            ...
            if(!ctx.counted)
            {
                BOOST_INTERLOCKED_INCREMENT(&flag.count);
                ctx.counted=true;
                long status=::boost_ksim::detail::interlocked_read_acquire(&flag.status);
            ...
            BOOST_VERIFY(!::boost_ksim::detail::win32::WaitForSingleObjectEx(
                             ctx.event_handle,::boost_ksim::detail::win32::infinite, 0));
        }
    }
//#endif
