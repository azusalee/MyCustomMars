...
#pragma warning(pop)
            ...
            void lock()
            {
                ...
                if(old_count&lock_flag_value)
                {
                    ...
                    do
                    {
                        unsigned const retval(win32::WaitForSingleObjectEx(sem, ::boost_ksim::detail::win32::infinite,0));
                        BOOST_VERIFY(0 == retval || ::boost_ksim::detail::win32::wait_abandoned == retval);
//                        BOOST_VERIFY(win32::WaitForSingleObject(
//                                         sem,::boost_ksim::detail::win32::infinite)==0);
...
#if defined BOOST_THREAD_USES_DATETIME
            bool timed_lock(::boost_ksim::system_time const& wait_until)
            {
                if(try_lock())
                {
                    return true;
                ...
                if(old_count&lock_flag_value)
                {
                    ...
                    do
                    {
                        if(win32::WaitForSingleObjectEx(sem,::boost_ksim::detail::get_milliseconds_until(wait_until),0)!=0)
                        {
        ...
        private:
            void* get_event()
            {
                void* current_event=::boost_ksim::detail::interlocked_read_acquire(&event);
