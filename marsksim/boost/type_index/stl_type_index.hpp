...
/// \brief Contains mars_boost_ksim::typeindex::stl_type_index class.
///
/// mars_boost_ksim::typeindex::stl_type_index class can be used as a drop-in replacement 
/// for std::type_index.
///
...
/// is defined mars_boost_ksim::typeindex::ctti is usually used instead of mars_boost_ksim::typeindex::stl_type_index.
...
inline std::string stl_type_index::pretty_name() const {
    static const char cvr_saver_name[] = "mars_boost_ksim::typeindex::detail::cvr_saver<";
    ...
    const mars_boost_ksim::core::scoped_demangled_name demangled_name(data_->name());

    const char* begin = demangled_name.get();
    if (!begin) {
        mars_boost_ksim::throw_exception(std::runtime_error("Type name demangling failed"));
    }
...
#else
    return mars_boost_ksim::hash_range(raw_name(), raw_name() + std::strlen(raw_name()));
#endif
}
...
inline stl_type_index stl_type_index::type_id() BOOST_NOEXCEPT {
    ...
    typedef BOOST_DEDUCED_TYPENAME mars_boost_ksim::remove_cv<no_ref_t>::type no_cvr_prefinal_t;
    ...
    #  if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) \
        ...
        typedef BOOST_DEDUCED_TYPENAME mars_boost_ksim::mpl::if_<
            mars_boost_ksim::is_signed<no_cvr_prefinal_t>,
            mars_boost_ksim::make_signed<no_cvr_prefinal_t>,
            mars_boost_ksim::mpl::identity<no_cvr_prefinal_t>
...
inline stl_type_index stl_type_index::type_id_with_cvr() BOOST_NOEXCEPT {
    typedef BOOST_DEDUCED_TYPENAME mars_boost_ksim::mpl::if_<
        mars_boost_ksim::mpl::or_<mars_boost_ksim::is_reference<T>, mars_boost_ksim::is_const<T>, mars_boost_ksim::is_volatile<T> >,
        detail::cvr_saver<T>,
        T
    >::type type;
