...
#include "marsksim/comm/strutil.h"
#include "marsksim/comm/string_cast.h"
...
static const int kComplexConnectksimTimeout = 5 * 1000; //ms
static const int kComplexConnectksimInterval = 3 * 1000; //ms
...
: req_(_req)
, worker_thread_(boost_ksim::bind(&HTTPDetector::__Run, this), XLOGGER_TAG "::HTTPDetector")
, callback_(NULL){
...
void HTTPDetector::__Detect() {
    ...
    int conn_timeout = std::min(kComplexConnectksimTimeout, (int)remain_timeout);
    xdebug2(TSF"@%_, con_timeout:%_", this, conn_timeout);
    ComplexConnectksim conn(conn_timeout, kComplexConnectksimInterval);
    ...
    if (INVALID_SOCKET==sock) {
        ...
        } else if (detect_tick.gettickspan() >= kComplexConnectksimTimeout) {
