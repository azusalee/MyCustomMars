...
class Spy {
  public:
    ...
    { return boost_ksim::any_cast<var>(m_variablemap.find(_key)->second); }
  ...
  private:
    void* m_this;
    std::map<const std::string, boost_ksim::any> m_variablemap;
};

class SpyCore {
  public:
    SINGLETON_INTRUSIVE_ksim(SpyCore, new SpyCore, delete);
