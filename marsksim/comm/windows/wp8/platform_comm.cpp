...
int getStatisticsNetType() {
    return Runtime2Cs_Comm::Singletonksim()->getStatisticsNetType();
}
...
bool getProxyInfo(int& /*port*/, std::string& strProxy, const std::string& /*_host*/) {
    ...
    ProxyInfo^ info = Runtime2Cs_Comm::Singletonksim()->getProxyInfo();
...
int getNetInfo() {
    return Runtime2Cs_Comm::Singletonksim()->getNetInfo();
}

bool getCurWifiInfo(WifiInfo& wifiInfo) {
    CurWifiInfo^ info = Runtime2Cs_Comm::Singletonksim()->getCurWifiInfo();
...
bool isNetworkConnected() {
    return Runtime2Cs_Comm::Singletonksim()->isNetworkConnected();
}
...
void ConsoleLog(const XLoggerInfo* _info, const char* _log) {
    Runtime2Cs_Comm::Singletonksim()->ConsoleLog((int)_info->level, char2PString(_info->tag), char2PString(_info->filename), char2PString(_info->func_name), _info->line, char2PString(_log));
}
