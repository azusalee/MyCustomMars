...
class Condition {
  public:
    ...
    int wait(ScopedLock& lock, unsigned long millisecond) {
        ...
        if (!atomic_cas32(&anyway_notify_, 0, 1))
            ret = condition_.timed_wait(lock.internal().internal(), boost_ksim::get_system_time() + boost_ksim::posix_time::milliseconds(millisecond));
  ...
  private:
    boost_ksim::condition condition_;
    Mutex mutex_;
