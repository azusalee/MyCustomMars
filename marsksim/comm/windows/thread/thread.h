...
typedef boost_ksim::thread*  thread_handler;
#else
#include <thr/threads.h>
...
class ThreadUtilksim {
  public:
    static void yield() {
#ifdef  USED_BOOST_THREAD_LIB
        boost_ksim::this_thread::yield();
#else
        thrd_yield();
#endif
    }
...
#ifdef  USED_BOOST_THREAD_LIB
        return boost_ksim::detail::win32::GetCurrentThreadId();
#else
    ...
    static int createThread(thread_handler& pth, THREAD_START_PROC proc, void* args) {
        ...
        pth = new boost_ksim::thread(proc, args);
        return thrd_success;
#else
...
class Thread {
  ...
  public:
    ...
    int start(bool* _newone = NULL) {
        ...
        int ret = ThreadUtilksim::createThread(m_runableref->m_th, &start_routine, (void*)m_runableref);
    ...
    int start_after(unsigned int after) {
        ...
        int ret = ThreadUtilksim::createThread(m_runableref->m_th, &start_routine_after, (void*)m_runableref);
        ASSERT(thrd_success == ret);
    ...
    int start_periodic(unsigned int after, unsigned int periodic) { // ms
        ...
        int ret = ThreadUtilksim::createThread(m_runableref->m_th, &start_routine_periodic, (void*)m_runableref);
        ASSERT(thrd_success == ret);
    ...
    void join() const {
        ...
        if (isruning()) {
            ...
            lock.unlock();
            ThreadUtilksim::join(m_runableref->m_th);
    ...
    thread_tid tid() const {
        return ThreadUtilksim::getThreadId(m_runableref->m_th);
  ...
  private:
    ...
    static void cleanup(void* arg) {
        ...
        if (!runableref->isjoined) {
            ...
            ThreadUtilksim::detach(const_cast<RunnableReference*>(runableref)->m_th);
        }
