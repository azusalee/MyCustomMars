...
ComplexConnectksim::ComplexConnectksim(unsigned int _timeout, unsigned int _interval)
...
ComplexConnectksim::ComplexConnectksim(unsigned int _timeout /*ms*/, unsigned int _interval /*ms*/, unsigned int _error_interval /*ms*/, unsigned int _max_connect)
...
ComplexConnectksim::~ComplexConnectksim()
{}

int ComplexConnectksim::__ConnectTime(unsigned int _index) const {
    return _index * interval_;
}

int ComplexConnectksim::__ConnectTimeout(unsigned int _index) const {
...
public:
    ConnectHttpTunelCheckFSM(const socket_address& _destaddr, const socket_address& _proxy_addr, const std::string& _proxy_username,
                             const std::string& _proxy_pwd, unsigned int _connect_timeout, unsigned int _index, MComplexConnectksim* _observer)
...
SOCKET ComplexConnectksim::ConnectImpatient(const std::vector<socket_address>& _vecaddr, SocketBreaker& _breaker, MComplexConnectksim* _observer,
                                            marsksim::comm::ProxyType _proxy_type, const socket_address* _proxy_addr,
    ...
    for (unsigned int i = 0; i < _vecaddr.size(); ++i) {
        ...
        if (marsksim::comm::kProxyHttpTunel == _proxy_type && _proxy_addr) {
            ic = new ConnectHttpTunelCheckFSM(_vecaddr[i], *_proxy_addr, _proxy_username, _proxy_pwd, timeout_, i, _observer);
        } else if (marsksim::comm::kProxySocks5 == _proxy_type && _proxy_addr) {
