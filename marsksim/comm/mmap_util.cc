...
bool IsMmapFileOpenSucc(const boost_ksim::iostreams::mapped_file& _mmmap_file) {
    return !_mmmap_file.operator !() && _mmmap_file.is_open();
}

bool OpenMmapFile(const char* _filepath, unsigned int _size, boost_ksim::iostreams::mapped_file& _mmmap_file) {
    ...
    boost_ksim::iostreams::basic_mapped_file_params<boost_ksim::filesystem::path> param;
    param.path = boost_ksim::filesystem::path(_filepath);
    param.flags = boost_ksim::iostreams::mapped_file_base::readwrite;

    bool file_exist = boost_ksim::filesystem::exists(_filepath);
    if (!file_exist) {
    ...
    if (!file_exist && is_open) {
        ...
        if (NULL == file) {
            _mmmap_file.close();
            boost_ksim::filesystem::remove(_filepath);
            return false;
        }
...
void CloseMmapFile(boost_ksim::iostreams::mapped_file& _mmmap_file) {
