...
struct TaskInfo {
public:
    TaskInfo(boost_ksim::intrusive_ptr<Wrapper> _wrapper, ::SocketPoll& _poll)
...
static void __Coro_Poll(int _msec, ::SocketPoll& _socket_poll) {
    ...
    boost_ksim::shared_ptr<mqksim::RunloopCond> cond = mqksim::RunloopCond::CurrentCond();
    ...
    if (cond && cond->type() == boost_ksim::typeindex::type_id<coroutine::RunloopCond>()) {
...
const boost_ksim::typeindex::type_info& RunloopCond::type() const {
    return boost_ksim::typeindex::type_id<coroutine::RunloopCond>().type_info();
}
