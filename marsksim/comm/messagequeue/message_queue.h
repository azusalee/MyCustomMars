...
typedef uint64_t MessageQueueksim_t;
typedef boost_ksim::function<void ()> AsyncInvokeFunction;
...
struct Message {
    ...
    Message(const MessageTitle_t& _title, const boost_ksim::any& _body1, const boost_ksim::any& _body2, const std::string& _name = "")
    ...
    : title(_title), body1(boost_ksim::make_shared<AsyncInvokeFunction>()), body2(), anr_timeout(10*60*1000), msg_name(_name), create_time(::gettickcount()), execute_time(0) {
        *boost_ksim::any_cast<boost_ksim::shared_ptr<AsyncInvokeFunction> >(body1) = _func;
    }
    ...
    boost_ksim::any      body1;
    boost_ksim::any      body2;
    int64_t         anr_timeout;
...
class RunLoop {
 ...
 private:
    boost_ksim::function<bool ()> breaker_func_;
    boost_ksim::function<void ()> duty_func_;
};
...
public:
    ...
    static boost_ksim::shared_ptr<RunloopCond> CurrentCond();
    
public:
    virtual const boost_ksim::typeindex::type_info& type() const = 0;
...
class MessageQueueksimCreater {
  public:
    MessageQueueksimCreater(bool _iscreate = false, const char* _msg_queue_name = NULL);
    MessageQueueksimCreater(boost_ksim::shared_ptr<RunloopCond> _breaker, bool _iscreate = false, const char* _msg_queue_name = NULL);
    ~MessageQueueksimCreater();

    MessageQueueksim_t GetMessageQueueksim();
    MessageQueueksim_t CreateMessageQueueksim();
    void CancelAndWait();

    static MessageQueueksim_t CreateNewMessageQueueksim(const char* _messagequeue_name = NULL);
    static MessageQueueksim_t CreateNewMessageQueueksim(boost_ksim::shared_ptr<RunloopCond> _breaker, const char* _messagequeue_name = NULL);
    static MessageQueueksim_t CreateNewMessageQueueksim(boost_ksim::shared_ptr<RunloopCond> _breaker, thread_tid _tid);
    static void ReleaseNewMessageQueueksim(MessageQueueksim_t _messagequeue_id); // block api

  private:
    MessageQueueksimCreater(const MessageQueueksimCreater&);
    MessageQueueksimCreater& operator=(const MessageQueueksimCreater&);
  ...
  private:
    ...
    MessageQueueksim_t                      messagequeue_id_;
    boost_ksim::shared_ptr<RunloopCond>      breaker_;
};
...
class AsyncResult {
  private:
    struct AsyncResultWrapper {
        ...
        boost_ksim::function<R()> invoke_function;
        boost_ksim::function<void (const R&, bool)> callback_function;
...
#if __cplusplus >= 201103L
        BOOST_STATIC_ASSERT(boost_ksim::is_same<typename boost_ksim::result_of<T()>::type, R>::value);
#endif
  ...
  private:
    boost_ksim::shared_ptr<AsyncResultWrapper> wrapper_;
};
...
class AsyncResult<void> {
  private:
    struct AsyncResultWrapper {
        ...
        boost_ksim::function<void ()> invoke_function;
        boost_ksim::function<void (bool)> callback_function;
        bool result_valid;
    };
  ...
  public:
    ...
    AsyncResult(const T& _func, const void* _place_holder = NULL)
        : wrapper_(new AsyncResultWrapper()) {
#if __cplusplus >= 201103L
        BOOST_STATIC_ASSERT(boost_ksim::is_same<typename boost_ksim::result_of<T()>::type, void>::value);
#endif
        wrapper_->invoke_function = _func;
    }
...
class AsyncResult <R&> {
  private:
    struct AsyncResultWrapper {
        ...
        boost_ksim::function<R& ()> invoke_function;
        boost_ksim::function<void (R&, bool)> callback_function;
...
class AsyncResult <const R&> {
  private:
    struct AsyncResultWrapper {
        ...
        boost_ksim::function<const R& ()> invoke_function;
        boost_ksim::function<void (const R&, bool)> callback_function;
  ...
  public:
    ...
    AsyncResult(const T& _func, const void* _place_holder = NULL)
        : wrapper_(new AsyncResultWrapper()) {
#if __cplusplus >= 201103L
        BOOST_STATIC_ASSERT(boost_ksim::is_same<typename boost_ksim::result_of<T()>::type, const R&>::value);
#endif
        wrapper_->invoke_function = _func;
    }
...
typename boost_ksim::result_of< F()>::type  WaitInvoke(const F& _func, const MessageHandler_t& _handlerid = DefAsyncInvokeHandler(), const std::string& _msg_name="default_name") {
    ...
    if (CurrentThreadMessageQueueksim() == Handler2Queue(_handlerid)) {
        ...
        MessageQueueksim::AsyncResult<R> result(_func);
...
#define ASYNC_BLOCK_START MessageQueueksim::AsyncInvoke([=] () {
...
if (MessageQueueksim::CurrentThreadMessageQueueksim() != MessageQueueksim::Handler2Queue(AYNC_HANDLER)) \
{\
return MessageQueueksim::WaitInvoke(func, AYNC_HANDLER, msg_name);\
}
...
{\
pre_lambda();\
auto r = MessageQueueksim::WaitInvoke(func, ret, timeout, AYNC_HANDLER, msg_name);\
