...
using namespace marsksim::comm;

boost_ksim::signals2::signal<void (bool _iOS_style, const check_content& _content)>& GetSignalksimCheckHit() {
	static boost_ksim::signals2::signal<void (bool _iOS_style, const check_content& _content)> SignalCheckHit;
    ...
    while (true) {
        ...
        if (iOS_style) {
            if (!sg_check_heap.empty() && (uint64_t)sg_check_heap.front().timeout <= sg_check_heap.front().used_cpu_time) {
                check_hit = true;
                GetSignalksimCheckHit()(true, sg_check_heap.front());
            ...
            if (!sg_check_heap.empty()  && sg_check_heap.front().end_time <= clock_app_monotonic()) {
                check_hit = true;
                GetSignalksimCheckHit()(false, sg_check_heap.front());
            }
        }
