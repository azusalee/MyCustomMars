...
#include "marsksim/boost/bind.hpp"
#include "marsksim/boost/ref.hpp"

#include "marsksim/comm/thread/lock.h"
#include "marsksim/comm/coroutine/coroutine.h"
#include "marsksim/comm/coroutine/coro_async.h"
...
bool getProxyInfo(int& port, std::string& strProxy, const std::string& _host) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&getProxyInfo, boost_ksim::ref(port), boost_ksim::ref(strProxy), _host));
...
bool getCurRadioAccessNetworkInfo(RadioAccessNetworkInfo& _raninfo) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&getCurRadioAccessNetworkInfo, boost_ksim::ref(_raninfo)));
...
DEFINE_FIND_STATIC_METHOD(KPlatformCommC2Java_getCurWifiInfo, KPlatformCommC2Java,
                          "getCurWifiInfo", "()Lcom/tencent/marsksim/comm/PlatformComm$WifiInfo;")
bool getCurWifiInfo(WifiInfo& wifiInfo) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&getCurWifiInfo, boost_ksim::ref(wifiInfo)));
...
DEFINE_FIND_STATIC_METHOD(KPlatformCommC2Java_getCurSIMInfo, KPlatformCommC2Java, "getCurSIMInfo",
                          "()Lcom/tencent/marsksim/comm/PlatformComm$SIMInfo;")
bool getCurSIMInfo(SIMInfo& simInfo) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&getCurSIMInfo, boost_ksim::ref(simInfo)));
...
DEFINE_FIND_STATIC_METHOD(KPlatformCommC2Java_getAPNInfo, KPlatformCommC2Java, "getAPNInfo", "()Lcom/tencent/marsksim/comm/PlatformComm$APNInfo;")
bool getAPNInfo(APNInfo& info) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&getAPNInfo, boost_ksim::ref(info)));
    ...
    jobject retObj = JNU_CallStaticMethodByName(env, cacheInstance->GetClass(env, KPlatformCommC2Java), "getAPNInfo", "()Lcom/tencent/marsksim/comm/PlatformComm$APNInfo;").l;
...
unsigned int getSignal(bool isWifi) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&getSignal, isWifi));
...
DEFINE_FIND_STATIC_METHOD(KPlatformCommC2Java_wakeupLock_new, KPlatformCommC2Java, "wakeupLock_new",
                          "()Lcom/tencent/marsksim/comm/WakerLock;")
void* wakeupLock_new() {
    xverbose_function();
...
void  wakeupLock_delete(void* _object) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&wakeupLock_delete, _object));
...
void  wakeupLock_Lock(void* _object) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&wakeupLock_Lock, _object));
...
void  wakeupLock_Lock_Timeout(void* _object, int64_t _timeout) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&wakeupLock_Lock_Timeout, _object, _timeout));
...
void  wakeupLock_Unlock(void* _object) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&wakeupLock_Unlock, _object));
...
bool  wakeupLock_IsLocking(void* _object) {
    ...
    if (coroutine::isCoroutine())
        return coroutine::MessageInvoke(boost_ksim::bind(&wakeupLock_IsLocking, _object));
