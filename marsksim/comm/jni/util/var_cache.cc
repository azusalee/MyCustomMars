...
VarCache* VarCache::Singletonksim() {
    if (instance_ == NULL) {
...
bool LoadClass(JNIEnv* env) {
    ...
    for (std::set<std::string>::iterator it = class_name_set.begin(); it != class_name_set.end(); ++it) {
        jclass clz = VarCache::Singletonksim()->GetClass(env, (*it).c_str());
...
bool LoadStaticMethod(JNIEnv* _env) {
    ...
    for (std::set<JniMethodInfo>::iterator iter = methoNameSet.begin(); iter != methoNameSet.end(); ++iter) {
        jmethodID mid = VarCache::Singletonksim()->GetStaticMethodId(_env, (*iter).classname.c_str(), (*iter).methodname.c_str(), (*iter).methodsig.c_str());
...
bool LoadMethod(JNIEnv* env) {
    ...
    for (std::set<JniMethodInfo>::iterator iter = methoNameSet.begin(); iter != methoNameSet.end(); ++iter) {
        jmethodID mid = VarCache::Singletonksim()->GetMethodId(env, (*iter).classname.c_str(), (*iter).methodname.c_str(), (*iter).methodsig.c_str());
